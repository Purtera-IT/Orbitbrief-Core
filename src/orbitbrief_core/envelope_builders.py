...
OWNERSHIP: this module lives in Orbitbrief-Core. Was previously at
``app/core/orbitbrief_core.py`` in parser-os; the parser-os copy is
now a deprecation shim that re-exports from here.

These builders consume parser-os types (CompileResult, EvidenceAtom,
EntityRecord, EvidenceEdge, EvidencePacket) but produce brief-layer
output that has no place in a parsing library.
...
# v58: these were byte-identical copies of parser-os's implementations.
# Two copies of one rule is two rules — they had already drifted apart in
# five OTHER builders (build_site_readiness lost 264 lines and with them the
# whole physical_site branch). Import the single owner instead; parser-os is
# already a hard dependency of this module.
from app.core.orbitbrief_core import (  # noqa: F401  (re-exported)
    _atom_authority_rank,
    _atom_type_str,
    _owner_slug_from_atom,
    _review_status_str,
    build_scope_truth,
