...
OWNERSHIP: this module lives in Orbitbrief-Core. parser-os retains a
shim at ``app/core/orbitbrief_envelope.py`` that re-exports from here
for back-compat. New code should import from
``orbitbrief_core.envelope`` directly.
...
# Envelope-layer analytical surfaces — moved here from parser-os
# (the original copy lived in app/core/orbitbrief_core.py until this
# repo took ownership of the brief layer).
from orbitbrief_core.envelope_builders import (
...
# parser-os types (the substrate contract). parser-os remains the
# source of truth for atom / entity / packet / source-ref schemas
# and for the orchestrating compile pipeline.
