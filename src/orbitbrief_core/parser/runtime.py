...
from orbitbrief_core.parser.registry import ParserRegistry, RegistryDispatchError, build_default_registry
...
def _get_registry(registry: ParserRegistry | None, *, strict: bool) -> ParserRegistry:
    if registry is not None:
        return registry
    return build_default_registry(allow_strategy_fallback=not strict)
...
def _run_adapter(
    *,
    ...
    registry.validate_plan(parse_plan, strict=strict)
...
def get_adapter_for_plan(parse_plan: ParsePlan, *, registry: ParserRegistry | None = None, strict: bool = True):
    ...
    active_registry.validate_plan(parse_plan, strict=strict)
...
def _run_strategy(
    *,
    ...
    for strategy in registry.strategy_chain(parse_plan, strict=strict):
...
def parse_and_packetize(
    *,
    ...
    """Runtime spine: router -> adapter -> strategy -> cue/authority -> graph -> packetizer."""
    active_registry = _get_registry(registry, strict=strict)
    diagnostics: list[str] = []
