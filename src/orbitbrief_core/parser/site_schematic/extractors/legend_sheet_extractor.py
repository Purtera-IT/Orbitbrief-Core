...
from orbitbrief_core.parser.site_schematic.legends.legend_parser import parse_legend_entries
from orbitbrief_core.parser.site_schematic.legends.outlet_type_parser import parse_outlet_type_definitions
from orbitbrief_core.parser.site_schematic.zoning.page_zones import build_page_regions


def extract_legend_sheet(
    *,
    page_index: int,
    text: str,
    ...
    regions = build_page_regions(page_index=page_index, text=text, sheet_type="legend_symbol", sheet_title=sheet_title)
    abbreviations = parse_abbreviation_entries(text, page_index=page_index)
    legend_entries = parse_legend_entries(text, page_index=page_index, overlay_tags=overlay_tags)
    outlet_defs = parse_outlet_type_definitions(text, page_index=page_index)
    ...
    return ExtractedPageArtifacts(
        ...
        outlet_type_definitions=outlet_defs,
