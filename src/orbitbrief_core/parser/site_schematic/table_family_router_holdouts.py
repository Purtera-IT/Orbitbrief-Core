...
HOLDOUT_TABLE_SIGNATURES: dict[str, list[list[str]]] = {
    "drawing_index": [
...
def score_holdout_table_family(
    ...
    *,
    ...
    combined = f"{header_text} | {cell_join}"
    scores: dict[str, float] = {}

    for kind, signatures in HOLDOUT_TABLE_SIGNATURES.items():
        score = 0.0
...
def choose_holdout_table_family(
    ...
    *,
    ...
    scores = score_holdout_table_family(
        header_text,
        cell_texts,
        sheet_family_hint=sheet_family_hint,
        region_kind_hint=region_kind_hint,
    )
    if not scores:
