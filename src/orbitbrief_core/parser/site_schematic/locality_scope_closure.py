...
def close_locality_scope_if_strong(
    ...
    *,
    ...
    """
    Deterministically upgrade unresolved locality only with strong evidence.
    """
    out = dict(scoped_note)
    scope_level = out.get("scope_level", out.get("scope_class", "unresolved"))
    if scope_level not in {"unresolved", "mixed", "candidate_requires_review"}:
        return out
    strength = 0
    reasons: list[str] = []
    ...
    if strength >= 3:
        ...
        metadata["locality_scope_closure"] = True
        metadata["locality_scope_closure_reasons"] = reasons
