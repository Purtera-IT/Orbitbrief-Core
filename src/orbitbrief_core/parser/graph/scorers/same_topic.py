...
from orbitbrief_core.parser.graph.neural_hooks import SameTopicRequest, ScoreResult


@dataclass(slots=True)
class SameTopicScoringService:
    """Optional backend scorer for same-topic candidates."""

    model_name: str = "same_topic:heuristic"
    backend: Callable[[SameTopicRequest], float | None] | None = None
    enabled: bool = True
    timeout_ms: int = 350
    ...
    def score(self, request: SameTopicRequest) -> ScoreResult:
        ...
        bounded = SameTopicRequest(
            left_span_id=request.left_span_id,
            right_span_id=request.right_span_id,
            left_text=str(request.left_text or "")[: max(32, self.max_context_chars)],
            right_text=str(request.right_text or "")[: max(32, self.max_context_chars)],
