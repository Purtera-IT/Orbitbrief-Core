...
def _extract_structured(document: Any, *, source_path: str | None) -> tuple[tuple[ProviderLayoutBlock, ...], tuple[ProviderTableRegion, ...]]:
    ...
    for node in candidate_nodes:
        ...
        metadata = {
            "provider": "mineru",
            "mineru_label": label_text or "unknown",
            "source_path": source_path,
            "degraded": False,
        }
        if role == "table":
            region_id = f"mineru_table:{page_index:04d}:{table_index:04d}"
            tables.append(
                ProviderTableRegion(
                    region_id=region_id,
            ...
            blocks.append(
                ProviderLayoutBlock(
                    block_id=f"mineru_block:{page_index:04d}:{block_index:04d}",
                    page_index=page_index,
                    bbox=bbox,
                    text=text,
                    role="table",
...
def _markdown_fallback(document: Any, *, source_path: str | None) -> tuple[ProviderLayoutBlock, ...]:
    ...
    for index, chunk in enumerate(chunks):
        ...
        out.append(
            ProviderLayoutBlock(
                block_id=f"mineru_fallback_block:{index:04d}",
                page_index=0,
                bbox=None,
                text=chunk,
                role=role,
                ...
                metadata={
                    "provider": "mineru",
                    "mineru_label": "markdown_fallback",
                    "source_path": source_path,
                    "degraded": True,
                },
            )
...
def extract_mineru_pdf_hypothesis(*, pdf_path: Path | None = None, pdf_bytes: bytes | None = None) -> ProviderPdfHypothesis | None:
    """Best-effort MinerU provider hypothesis for structured PDF blocks."""
    try:
        from mineru.document_converter import DocumentConverter  # type: ignore
    except Exception:
        try:
            from mineru import DocumentConverter  # type: ignore
        except Exception:
            return None
    if pdf_path is None:
        return None
    ...
    blocks, tables = _extract_structured(document, source_path=str(pdf_path))
    if blocks:
        confidence = min(0.95, 0.70 + min(0.20, len(blocks) * 0.01) + min(0.10, len(tables) * 0.012))
    ...
    fallback = _markdown_fallback(document, source_path=str(pdf_path))
    if not fallback:
        return None
    return ProviderPdfHypothesis(
        hypothesis_id="hypothesis:mineru",
        source="mineru",
        page_blocks=fallback,
        table_regions=(),
        confidence=0.60,
        metadata={"provider": "mineru", "degraded": True, "source_path": str(pdf_path)},
    )
