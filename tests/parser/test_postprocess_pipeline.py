...
def test_postprocess_does_not_group_array_field_paths_as_contradictions() -> None:
    output = ExtractorOutput(
        candidates=(
            ClaimCandidate(
                claim_id="c1",
    ...
    result = run_postprocess(extractor_output=output, policy=policy)
    assert len(result.surviving_claims) == 2
    assert len(result.contradiction_groups) == 0
    assert not any(flag.code == "conflicting_evidence" for flag in result.review_flags)
