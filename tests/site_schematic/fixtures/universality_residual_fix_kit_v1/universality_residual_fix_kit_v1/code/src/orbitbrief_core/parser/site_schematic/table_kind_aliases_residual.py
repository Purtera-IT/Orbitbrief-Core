...
RESIDUAL_HOLDOUT_TABLE_ALIASES = {
    "drawing_index": [
...
def score_residual_holdout_table_aliases(header_text: str, row_texts: Iterable[str]) -> Dict[str, float]:
    header_text = (header_text or "").lower()
    joined = header_text + " | " + " | ".join((r or "").lower() for r in row_texts)
    scores: Dict[str, float] = {}
    for kind, sigs in RESIDUAL_HOLDOUT_TABLE_ALIASES.items():
        score = 0.0
        for sig in sigs:
