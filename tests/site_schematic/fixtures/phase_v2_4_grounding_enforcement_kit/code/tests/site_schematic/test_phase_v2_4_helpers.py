...
def test_family_coverage():
    c = compute_family_coverage(
        expected_families=["a","b","c"],
        grounded_families=["a","b"],
        hardpage_grounded_families=["a"],
    )
    assert c.expected_family_grounded_coverage_rate == 2/3
    assert c.hardpage_family_grounded_coverage_rate == 1/3


def test_room_truth_audit():
    out = audit_room_device_truth(
    ...
    assert out.evidence_truth_ok is True


def test_connector_truth_audit():
    out = audit_connector_truth(
...
def test_hardpage_gate():
    ...
    assert out.hardpage_requirement_truth_ok is True


def test_sample_rows():
    rows = [{"grounded_family": "camera", "grounding_state": "grounded"}]
    s = select_grounding_sample_rows(rows, limit=5)
    assert len(s) == 1
