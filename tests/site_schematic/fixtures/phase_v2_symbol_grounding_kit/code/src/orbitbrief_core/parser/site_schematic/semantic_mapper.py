...
def build_legend_grounding_dictionary(
    *,
    page_index: int,
    legend_entries: Iterable[Any],
) -> List[LegendGroundingEntry]:
    ontology_aliases = {}
    for entry in DEFAULT_V2_ONTOLOGY:
        for alias in entry.aliases:
            ontology_aliases[alias.lower()] = entry.family
    ...
    for idx, entry in enumerate(legend_entries):
        if isinstance(entry, dict):
            ...
            source_cell_ids = getattr(entry, "source_cell_ids", [])
            bbox = getattr(entry, "bbox", None)

        lowered = label.lower()
        family = "unknown_symbol_group"
        for alias, fam in ontology_aliases.items():
            if alias in lowered:
                family = fam
                break

        grounded.append(LegendGroundingEntry(
