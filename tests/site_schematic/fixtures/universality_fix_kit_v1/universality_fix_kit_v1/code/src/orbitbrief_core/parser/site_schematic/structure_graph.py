...
def infer_columns_from_blocks(
    ...
    # sort by x center and cluster with a gutter threshold
...
def build_page_structure_graph(
    *,
    ...
    detail_regions: Optional[Iterable[Any]] = None,
    subregions: Optional[Iterable[Any]] = None,
    pseudo_pages: Optional[Iterable[Any]] = None,
    ...
    for t_idx, table in enumerate(universal_tables):
        ...
        graph.add_node(StructureNode(
            ...
            metadata={
                ...
                "source_mode": _get(table, "source_mode", ""),
            },
        ))
        for row_idx, row in enumerate(_get(table, "rows", []) or []):
            ...
            for cell_idx, cell in enumerate(_get(row, "cells", []) or []):
                ...
                graph.add_node(StructureNode(
                    ...
                    bbox=cell_bbox,
                    label=(_get(cell, "text", "") or "")[:120],
                    confidence=float(_get(cell, "confidence", 0.0) or 0.0),
                    metadata={
                        "table_id": t_id,
                        "row_id": row_id,
                        "col_index": _get(cell, "col_index", cell_idx),
                    },
                ))
                graph.add_edge(StructureEdge(src_id=cell_id, dst_id=row_id, edge_kind="cell_in_row", confidence=1.0))

    # Regions / details / subregions / pseudo pages
    def _add_region_nodes(objs: Optional[Iterable[Any]], kind_name: str, edge_kind: str) -> None:
        if not objs:
            return
        for idx, obj in enumerate(objs):
            rid = _get(obj, "region_id", _get(obj, "detail_region_id", _get(obj, "subregion_id", _get(obj, "pseudo_page_id", f"{kind_name}:{page_index}:{idx}"))))
    ...
    _add_region_nodes(regions, "region", "in_region")
    _add_region_nodes(detail_regions, "detail_region", "in_detail_region")
    _add_region_nodes(subregions, "subregion", "in_subregion")
    _add_region_nodes(pseudo_pages, "pseudo_page", "in_pseudo_page")

    # Region membership by overlap
    for node in block_nodes + graph.nodes_by_kind("table"):
        if not node.bbox:
            continue
        for region in graph.nodes_by_kind("region") + graph.nodes_by_kind("detail_region") + graph.nodes_by_kind("subregion") + graph.nodes_by_kind("pseudo_page"):
            if not region.bbox:
                continue
