...
SHEET_FAMILY_KEYWORDS = {
    ...
    "floorplan": ["floor plan", "overall", "part plans", "enlarged guestroom layouts"],
}
...
def build_sheet_archetype_hints(structure_graph: Any) -> SheetArchetypeHint:
    ...
    text_pool = []
    for n in nodes:
        label = _lower(getattr(n, "label", ""))
        meta_text = _lower((getattr(n, "metadata", {}) or {}).get("text", ""))
        if label:
    ...
    # sheet id candidates: uppercase + digits-ish strings often seen in labels
    for piece in text_pool:
        for token in piece.replace("(", " ").replace(")", " ").replace("-", " ").split():
            raw = token.upper()
            if any(ch.isdigit() for ch in raw) and len(raw) >= 3:
                if raw not in hint.sheet_id_candidates:
                    hint.sheet_id_candidates.append(raw)

    for family, kws in SHEET_FAMILY_KEYWORDS.items():
        score = 0.0
        for kw in kws:
            if kw in joined:
                score += 1.0
    ...
    if "floorplan" in hint.family_scores and "drawing_index" in hint.family_scores:
        hint.family_scores["floorplan"] += 0.25
