...
def test_expected_family_deriver():
    fams = derive_expected_families_from_packet_local_text(
    ...
    assert "wireless_access_point" in fams


def test_required_hardpages():
    req = derive_required_hardpages(
...
def test_family_coverage_truth():
    cov = compute_family_coverage_truth(
        packet_expected_families=["a","b","c"],
        grounded_families=["a","b"],
        hardpage_expected_families=["a","b"],
        hardpage_grounded_families=["a"],
    )
    assert cov.expected_family_grounded_coverage_rate == 2/3
    assert cov.hardpage_family_grounded_coverage_rate == 1/2


def test_grounded_family_derivation():
    fam = derive_grounded_family(
        legend_text="WAP",
        page_type="riser_diagram",
        connector_context_score=0.6,
        allowed_families=["wireless_access_point","riser_endpoint"],
    )
    assert fam in {"wireless_access_point","riser_endpoint"}


def test_hardpage_gate():
