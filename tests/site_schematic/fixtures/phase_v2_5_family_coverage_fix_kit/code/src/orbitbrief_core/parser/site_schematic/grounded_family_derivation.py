...
def derive_grounded_family(
    *,
    ...
    text = " | ".join((legend_text or "", mapped_semantic_text or "", outlet_definition_text or "", page_title or "")).lower()
    allowed = set(allowed_families)
    matches = []
    for kw, fam in KEYWORD_TO_FAMILY.items():
        if kw in text:
            if not allowed or fam in allowed:
                matches.append(fam)
    if matches:
        return matches[0]
    # Weak fallback from page type context only when local evidence is non-trivial.
