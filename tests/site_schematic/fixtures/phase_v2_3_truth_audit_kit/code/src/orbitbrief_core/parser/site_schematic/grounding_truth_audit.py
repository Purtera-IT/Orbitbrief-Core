...
def audit_packet_truth_signals(
    *,
    ...
    if candidate_symbol_count > 0 and grounded_symbol_count == candidate_symbol_count and unresolved_symbol_count == 0:
        # only suspicious if row-level evidence also looks unnaturally uniform
        if grounded_rows:
    ...
    if connector_topology_candidate_rate < 0.1 and connector_grounding_quality_rate >= 0.95:
