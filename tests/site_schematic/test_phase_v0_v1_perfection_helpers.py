...
def test_modality_calibration() -> None:
    result = calibrate_modality_decision(
    ...
    assert result.modality in {"hybrid", "vector_rich"}
...
def test_primitive_validation() -> None:
    ...
    validations = [validate_vector_primitive(row) for row in primitives]
    assert any(row.valid for row in validations)


def test_packet_quality_summary() -> None:
    summary = summarize_packet_v0_v1(
        packet_id="packet",
        page_modality_rows=[{"modality": "vector_rich", "ambiguous": False}],
        primitive_graph_rows=[
            {
                "primitive_count": 3,
                "validated_primitive_count": 3,
                "leader_candidate_count": 1,
                "dimension_candidate_count": 1,
            }
        ],
    )
    assert summary.modality_fail is False
    assert summary.primitive_graph_fail is False
