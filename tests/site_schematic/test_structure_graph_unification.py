...
def test_structure_graph_builds_columns_tables_and_regions():
    ...
    graph = build_page_structure_graph(
        page_index=0,
    ...
    assert graph.diagnostics["column_count"] >= 1
    assert graph.diagnostics["table_count"] == 1
    assert graph.diagnostics["region_count"] == 1
...
def test_sheet_hints_and_table_kind_router():
    ...
    hints = build_sheet_archetype_hints(graph)
    assert "legend_symbol" in hints.family_scores
    kind, _ = infer_table_kind_from_structure_graph(universal_tables[0], graph)
...
def test_note_scope_uses_structure_graph():
    ...
    decision = infer_note_scope_with_structure_graph({"block_id": "note1"}, graph, detail_tokens=["D1"])
