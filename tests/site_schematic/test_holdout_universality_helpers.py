...
from orbitbrief_core.parser.site_schematic.holdout_titleblock_profiles import (
    score_sheet_text_against_holdout_profiles,
)
from orbitbrief_core.parser.site_schematic.observation_escalation_policy import (
    choose_page_escalation_policy,
)
from orbitbrief_core.parser.site_schematic.table_family_router_holdouts import (
    choose_holdout_table_family,
)
...
def test_table_family_router_picks_symbol_legend():
    family, _scores = choose_holdout_table_family(
        "SYMBOLS & LEGENDS",
...
def test_note_scope_classifier_prefers_column_or_detail():
    lanes = infer_holdout_columns(
        [
            {"block_id": "n1", "bbox": [0, 0, 180, 120], "text": "left"},
            {"block_id": "n2", "bbox": [220, 0, 390, 120], "text": "right"},
        ],
        page_width=400,
    )
    decision = classify_note_scope_with_columns(
        {"block_id": "n1", "bbox": [5, 10, 160, 60], "text": "NOTE"},
        column_lanes=lanes,
    )
    assert decision["scope_class"] in {"column_local", "page_global"}
