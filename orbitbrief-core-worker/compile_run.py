...
def _download_question_feedback(client: BlobServiceClient, deal_id: str, dest: Path) -> Path | None:
    """Merge deal + org-wide PM question feedback into one JSONL for Core.

    Sources (best-effort, missing blobs are fine):
      deals/{deal_id}/orbitbrief/latest/question_feedback.jsonl
      orbitbrief/learning/question_feedback.jsonl
    """
    lines: list[str] = []
    for blob in (
        f"deals/{deal_id}/orbitbrief/latest/question_feedback.jsonl",
        "orbitbrief/learning/question_feedback.jsonl",
    ):
        try:
            raw = (
                client.get_blob_client(_container(), blob)
                .download_blob()
                .readall()
...
def compile_orbitbrief_run(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="ob-core-") as tmp:
        ...
        feedback_path = _download_question_feedback(
            client, deal_id, work / "question_feedback.jsonl"
        )
        if feedback_path is not None:
            os.environ["ORBITBRIEF_QUESTION_FEEDBACK_PATH"] = str(feedback_path)
            log.info(
                "question feedback loaded deal=%s path=%s",
                deal_id,
                feedback_path,
            )
        if mirror_latest:
